...
All review calls use **`codex exec`** (`mcp__codex__codex`) with `reasoning_effort: xhigh`.

This is the default for ALL skills. No parameter, no config, no effort level changes this.

## Optional: GPT-5.5 Pro via Oracle

When the user explicitly passes `— reviewer: oracle-pro`, route the review through Oracle MCP instead of `codex exec`.

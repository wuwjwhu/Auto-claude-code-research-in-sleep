...
| paper-illustration-cpa-image2 finalization | `~/.claude/skills/paper-illustration-cpa-image2/paper_illustration_cpa_image2.py preflight --workspace <cwd>` returns `ok=true` | `paper_illustration_cpa_image2.py` (`preflight`, `render`, `finalize`, `verify`) | `figures/ai_generated/figure_vN.png`, `render_vN.json`, `figure_final.png`, `latex_include.tex`, `review_log.json` | Step 0 checklist in `paper-illustration-cpa-image2` | `paper_illustration_cpa_image2.py finalize --workspace <cwd> --best-image <png>` | `paper_illustration_cpa_image2.py verify` (diagnostic, exit 1 on missing artifacts) |

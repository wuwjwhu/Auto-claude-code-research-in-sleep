...
- **REVIEWER_BIAS_GUARD = true** — When `true`, every review round uses a fresh `codex exec` invocation with no prior review context. Never carry forward prior review context for review rounds. Set to `false` only for deliberate debugging of the legacy behavior. **Empirical evidence (April 2026):** carrying review context forward with "since last round we did X" prompts inflated scores from real 3/10 → fake 8/10 across 5 rounds; switching to fresh reviews recovered the true 3/10 assessment.
...
If `REVIEWER_BIAS_GUARD = true` (default), use a **fresh** `codex exec` review for Round 2. Do not reuse prior review context, and do not include Round 1 fix summaries in the reviewer prompt.
...
If `REVIEWER_BIAS_GUARD = false` (legacy debugging only), you may use a context-carrying follow-up review, but this is **not** the recommended path.
...
- **Reviewer independence (Round 2+)**: when `REVIEWER_BIAS_GUARD = true` (default), use a **fresh** `codex exec` review for every review round; never include "since last round" / fix summaries in the prompt. See the Reviewer Independence Protocol section above.

...
- **REVIEWER_MODEL = `gpt-5.5`** — Model used via `codex exec` for brainstorming and review. Must be an OpenAI model (e.g., `gpt-5.5`, `o3`, `gpt-4o`).
...
if research-wiki/ exists:
    ...
    Log (only if $WIKI_SCRIPT resolved):
        [ -n "$WIKI_SCRIPT" ] && python3 "$WIKI_SCRIPT" log research-wiki/ "idea-creator wrote N proposals/ideas (M shortlisted/recommended, K eliminated)"
